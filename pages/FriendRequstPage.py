...
class FriendRequestsPage(BasePage):
    ...
    # ── networking helpers ───────────────────────────────────
    def _send(self, obj):   # encrypt just like other pages
    ...
    def _recv(self):
        ...
        return json.loads(raw.decode())      # ← no RSA decryption
    ...
    def _refresh(self):
        ...
        # build a button for each request (accept / reject)
        self.buttons = []
        y = 220
    ...
    def _reply(self, from_user, accept):
        self._send({"type": "respond_friend_request",
                    "from_user": from_user,
                    "accept": accept})
        self._refresh()        # reload list after action
