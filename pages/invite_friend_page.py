# ────────────────────────────────────────────────────────────────────
#  invite_friend_page.py  – send a *proper* friend-game request
#                           *patched 23-May-2025*
#      • uses the packet name the server expects:  "send_game_request"
#      • server figures out the sender, so we no longer need the
#        client-side username check that was showing the red error.
# ────────────────────────────────────────────────────────────────────
import pygame, json
        ...
        for ev in events:
            if ev.type == pygame.MOUSEBUTTONDOWN:
                ...
                # INVITE -----------------------------------------------
                    ...
                    # remember context for WaitingPageFriend
                    gs = self.game_state
                    ...
                    # blocking read for small ACK (single packet)
