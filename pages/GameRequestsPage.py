...
class GameRequestsPage(BasePage):
    ...
    def __init__(self, manager, client, key):
        ...
        #   ↓↓↓ new state flags ↓↓↓
        self.awaiting_list = True        # waiting for server reply?
        self.partial       = ""          # buffer for split packets

        self.notice, self.notice_time = "", 0
        self.pending  = []              # list[dict] of requests
        self.buttons  = []              # (sender, tfmt, yes_btn, no_btn)
    ...
    def _ask_server_for_list(self):
        ...
        self.client.setblocking(False)         # <- key line

    # build / rebuild button objects
    ...
    # generic multi-packet splitter (same as WaitingPageFriend)
    @staticmethod
