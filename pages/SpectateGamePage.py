...
class SpectateGamePage(BasePage):
    ...
    def __init__(self, manager, client, key, game_id, fen, turn):
        ...
        if fen:
            # use the engine helper to load FEN into the internal board
        ...
        # networking thread to listen for board updates
        self.partial = ""
        self.running = True
    ...
    def _handle_packet(self, msg):
        if msg.get("type") == "opponent_move":
            fen = msg.get("fen")
            if fen:
                # load full snapshot if present
                self.chess.engine.set_fen(self.chess.board, fen)
            else:
                # fall-back: apply incremental move
                self.chess.make_move(tuple(msg["from"]), tuple(msg["to"]), self.current_turn)

            # flip turn for display
    ...
    def update(self):
        pass  # spectator doesn't tick clocks here
