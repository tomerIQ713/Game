# ────────────────────────────────────────────────────────────────────
#  play_page.py  – lobby with colour selector for Engine games
#                  *patched 23-May-2025*
#      • remembers the chosen time-format / game-type when the
#        user picks  “Play a friend”  (needed by InviteFriendPage)
# ────────────────────────────────────────────────────────────────────
import pygame, json
...
class PlayPage(BasePage):
    ...
    def __init__(self, manager, client, key):
        ...
        # DIFFICULTY (engine depth) – visible only for Computer
        self.diff_radio = [
        ...
        # COLOUR choice – visible only for Computer
        self.color_radio = [
    ...
    def handle_events(self, events):
        ...
        # update radio groups every frame
        ...
        for ev in events:
            if ev.type == pygame.MOUSEBUTTONDOWN:
                ...
                if self.start_btn.checkForInput(mouse):
                    ...
                    # 2. time control (unless Spectate / Computer)
                    ...
                    # 3. branch by game type
                    # ──────────────────────────────────────────
                    if gtype == "Random":
                        self._enc_send({"type": "request_game",
                                        "time": tlabel,
                                        "game_type": "Random"})
                    ...
                    if gtype == "Play a friend":
                        # Remember selections for the next page
                    ...
                    self.manager.set_current_page(
                        ...
                        current_turn="white",       # engine always starts as White
                        game_id="local",
                        vs_engine=True,
                        engine_depth=depth)
                    return  # done
